...
Revision ID: 93ad70d4308c
Revises: 
Create Date: 2026-01-25 13:08:07.083224
...
def upgrade() -> None:
    ...
    op.drop_index(op.f('transactions_transaction_receiver_id_3266914d'), table_name='transactions_transaction')
    op.drop_index(op.f('transactions_transaction_sender_id_4b6b0818'), table_name='transactions_transaction')
    ...
    op.drop_index(op.f('accounts_account_account_number_441b7708_like'), table_name='accounts_account', postgresql_ops={'account_number': 'varchar_pattern_ops'})
    op.drop_index(op.f('accounts_account_user_id_d61b40c6'), table_name='accounts_account')
...
def downgrade() -> None:
    ...
    sa.Column('user_id', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['auth_user.id'], name=op.f('accounts_account_user_id_d61b40c6_fk_auth_user_id'), initially='DEFERRED', deferrable=True),
    sa.PrimaryKeyConstraint('id', name=op.f('accounts_account_pkey')),
    sa.UniqueConstraint('account_number', name=op.f('accounts_account_account_number_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    op.create_index(op.f('accounts_account_user_id_d61b40c6'), 'accounts_account', ['user_id'], unique=False)
    op.create_index(op.f('accounts_account_account_number_441b7708_like'), 'accounts_account', ['account_number'], unique=False, postgresql_ops={'account_number': 'varchar_pattern_ops'})
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('receiver_id', sa.BIGINT(), autoincrement=False, nullable=True),
    sa.Column('sender_id', sa.BIGINT(), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['receiver_id'], ['accounts_account.id'], name=op.f('transactions_transac_receiver_id_3266914d_fk_accounts_'), initially='DEFERRED', deferrable=True),
    sa.ForeignKeyConstraint(['sender_id'], ['accounts_account.id'], name=op.f('transactions_transac_sender_id_4b6b0818_fk_accounts_'), initially='DEFERRED', deferrable=True),
    sa.PrimaryKeyConstraint('id', name=op.f('transactions_transaction_pkey'))
    )
    op.create_index(op.f('transactions_transaction_sender_id_4b6b0818'), 'transactions_transaction', ['sender_id'], unique=False)
    op.create_index(op.f('transactions_transaction_receiver_id_3266914d'), 'transactions_transaction', ['receiver_id'], unique=False)
